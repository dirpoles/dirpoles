...
function stats_referencias_admin()
{
    $modelo = new ReferenciasModel();
    header('Content-Type: application/json');
    try {
        // Marcamos como admin para que obtenga el total global del sistema
        $es_admin = in_array($_SESSION['tipo_empleado'], ['Administrador', 'Superusuario']);
        $modelo->__set('es_admin', $es_admin);
        $modelo->__set('id_empleado', $_SESSION['id_empleado']);
        ...
        echo json_encode([
            'exito' => true,
            'total' => $stats['totales'] // Enviamos como 'total' para que coincida con data.total en JS
        ]);
    } catch (Throwable $e) {
        echo json_encode([
            'exito' => false,
