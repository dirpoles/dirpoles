window.onload = function () {
  document.getElementById("btn-ayuda").addEventListener("click", function () {
    const driverObj = window.driver.js.driver({
      ...
      steps: [
        ...
        {
          element: "#tipo-discapacidad-input",
          popover: {
        ...
        },
        {
          ...
          popover: {
            ...
            description:
              "Selecciona el nivel de afectación: Leve, Moderado o Grave.",
            align: "center",
          },
        },
        {
          element: "#medicamentos-input",
          popover: {
            title: "Medicamentos",
            description:
              "Lista los medicamentos que toma actualmente, incluyendo dosis y frecuencia si se conoce.",
            align: "center",
          },
        },
        {
          element: "#habilidades-funcionales-input",
          popover: {
        ...
        {
          element: "#requiere-asistencia-input",
          popover: {
            title: "Asistencia Requerida",
            description:
              "Indica si el beneficiario necesita asistencia personal para sus actividades.",
            align: "center",
          },
        },
        {
          element: "#dispositivo-asistencia-input",
          popover: {
            title: "Dispositivo de Asistencia",
            description:
              "Especifica los dispositivos que utiliza el beneficiario, como silla de ruedas, audífonos o lentes.",
            align: "center",
          },
        },
        {
          element: "#carnet-discapacidad-input",
          popover: {
            title: "Carnet de Discapacidad",
            description:
              "Ingresa el número del carnet de discapacidad oficial si el beneficiario posee.",
            align: "center",
          },
        },
        {
          ...
          popover: {
            title: "Observaciones",
            description:
              "Agrega notas importantes o detalles adicionales sobre la discapacidad. Este campo es obligatorio.",
            align: "center",
          },
        },
        {
          element: "#recomendaciones-input",
          popover: {
            title: "Recomendaciones",
            description:
              "Proporciona sugerencias para el manejo, apoyos necesarios o adaptaciones para el beneficiario.",
            align: "center",
          },
        },
      ],
  ...
  const validarTipoDiscapacidad = () =>
    validarSelect(elements.tipo_discapacidad, true);
  const validarDiscEspecifica = () =>
    validarTexto(elements.disc_especifica, 3, 200, false);
  const validarDiagnostico = () =>
    validarTexto(elements.diagnostico, 5, 255, true);
  const validarGrado = () => validarSelect(elements.grado, true);
  const validarMedicamentos = () =>
    validarTexto(elements.medicamentos, 3, 255, false);
  const validarHabilidades = () =>
    validarTexto(elements.habilidades_funcionales, 5, 255, true);
  const validarRequiereAsistencia = () =>
    validarSelect(elements.requiere_asistencia, false); // No es required en HTML
  const validarDispositivo = () =>
    validarTexto(elements.dispositivo_asistencia, 3, 255, false);
  ...
  function validarCarnetDiscapacidad() {
    ...
    if (!/^\d+$/.test(numero)) {
      showError(
        elements.carnet_discapacidad,
        "El carnet debe contener solo números después de D-",
      );
      return false;
    }
    ...
    if (numero.length > 20) {
      // Ajustado a 20 según maxlength del HTML, usuario dijo 10 pero input tiene 20
      showError(
        elements.carnet_discapacidad,
        "El carnet debe tener menos de 20 dígitos",
      );
      return false;
    }
  ...
  const validarObservaciones = () =>
    validarTexto(elements.observaciones, 5, 500, true); // Textarea suele permitir más
  const validarRecomendaciones = () =>
    validarTexto(elements.recomendaciones, 5, 500, false);
  ...
  elements.tipo_discapacidad.addEventListener(
    "change",
  ...
  elements.habilidades_funcionales.addEventListener(
    "input",
    validarHabilidades,
  );
  elements.requiere_asistencia.addEventListener(
    "change",
    validarRequiereAsistencia,
  );
  elements.dispositivo_asistencia.addEventListener("input", validarDispositivo);
  elements.carnet_discapacidad.addEventListener(
    "input",
