...
function cargarData() {
    fetch(BASE_URL + "reportes_referencias_data")
        ...
        .catch(err => {
            console.error(err);
            Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo cargar la data de referencias.' });
        });
}
...
function filtrarYRenderizar() {
    ...
    if (filtered.length === 0) {
        Swal.fire({ icon: 'warning', title: 'Sin resultados', text: 'No se encontraron referencias con esos filtros.' });
        document.getElementById("contenedor_general").style.display = "none";
        return;
    }
    ...
    Swal.fire({
        ...
        timer: 3000, timerProgressBar: true, confirmButtonText: 'Entendido', confirmButtonColor: '#4e73df'
    });
...
function renderCharts(data) {
    const config = (title, labels, vals) => buildConfig(currentChartType, labels, vals, title);
    ...
    charts.G = new Chart(document.getElementById("chartGenero").getContext("2d"), config("Género", dG.labels, dG.values));
    charts.E = new Chart(document.getElementById("chartE").getContext("2d"), config("Estado", dE.labels, dE.values));
    charts.AO = new Chart(document.getElementById("chartAO").getContext("2d"), config("Área Origen", dAO.labels, dAO.values));
    charts.AD = new Chart(document.getElementById("chartAD").getContext("2d"), config("Área Destino", dAD.labels, dAD.values));
    charts.EO = new Chart(document.getElementById("chartEO").getContext("2d"), config("Empl. Origen", dEO.labels, dEO.values));
    charts.ED = new Chart(document.getElementById("chartED").getContext("2d"), config("Empl. Destino", dED.labels, dED.values));
}
...
function buildConfig(type, labels, values, title) {
    ...
    if (type === 'bar') {
        config.options.scales = {
            yAxes: [{
                ticks: { beginAtZero: true, precision: 0, callback: v => (v % 1 === 0) ? v : '' }
            }]
        };
...
function actualizarVisibilidadCharts() {
    ...
    else if (v === "estatus") document.getElementById("wrapper-chartE").style.display = "";
    else if (v === "areaO") document.getElementById("wrapper-chartAO").style.display = "";
    else if (v === "areaD") document.getElementById("wrapper-chartAD").style.display = "";
    else if (v === "empO") document.getElementById("wrapper-chartEO").style.display = "";
    else if (v === "empD") document.getElementById("wrapper-chartED").style.display = "";
}
...
function inicializarDataTable() {
    tableGeneral = $("#tabla_general").DataTable({
        ...
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', text: '<i class="fas fa-file-excel"></i> Excel', title: 'Reporte Referencias', className: 'btn btn-success btn-sm' },
            {
                extend: 'pdfHtml5', text: '<i class="fas fa-file-pdf"></i> PDF', title: 'Reporte Referencias', className: 'btn btn-danger btn-sm',
