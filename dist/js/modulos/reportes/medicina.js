...
let dataMorbilidad = [], dataInventario = [];
let lastFilteredMed = [], lastFilteredInv = [];

// Charts Morbilidad
let chartG = null, chartP = null, chartTS = null;
// Charts Inventario
let chartE = null, chartTI = null, chartPRE = null;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("tipo_reporte").addEventListener("change", function () {
        ...
        // Toggle filtros
        document.querySelectorAll(".filtro-morb").forEach(el => el.style.display = tipo === "morbilidad" ? "" : "none");
        document.querySelectorAll(".filtro-inv").forEach(el => el.style.display = tipo === "inventario" ? "" : "none");
    ...
    document.getElementById("form-reporte").addEventListener("submit", function (e) {
        ...
        if (!tipo) {
            Swal.fire({ icon: "error", title: "Formulario inválido", text: "Debe seleccionar un tipo de reporte.", confirmButtonText: "Entendido" });
            return;
        }
    ...
    document.getElementById("btn-limpiar").addEventListener("click", function () {
        ...
        document.getElementById("contenedor_medicina").style.display = "none";
        document.getElementById("contenedor_inventario").style.display = "none";
        document.querySelectorAll(".filtro-morb, .filtro-inv").forEach(el => el.style.display = "none");
        lastFilteredMed = []; lastFilteredInv = [];
        if (tableMed) tableMed.clear().draw();
        if (tableInv) tableInv.clear().draw();
    });

    // Chart selectors - Morbilidad
    document.getElementById("select-grafica-med").addEventListener("change", actualizarVisibilidadMed);
    document.getElementById("select-tipo-chart-med").addEventListener("change", function () {
        chartTypeMed = this.value;
        if (lastFilteredMed.length > 0) renderChartsMed(lastFilteredMed);
    });

    // Chart selectors - Inventario
    document.getElementById("select-grafica-inv").addEventListener("change", actualizarVisibilidadInv);
    document.getElementById("select-tipo-chart-inv").addEventListener("change", function () {
...
function cargarDataMorbilidad() {
    fetch(BASE_URL + "reportes_medicina_morbilidad_data")
        ...
        .catch(err => { console.error(err); Swal.fire({ icon: "error", title: "Error de conexión", text: "No se pudo obtener los datos de morbilidad médica." }); });
}
...
function cargarDataInventario() {
    fetch(BASE_URL + "reportes_medicina_inventario_data")
        ...
        .catch(err => { console.error(err); Swal.fire({ icon: "error", title: "Error de conexión", text: "No se pudo obtener los datos de inventario médico." }); });
}
...
function poblarSelectsMorb(data) {
    ...
    [...new Set(data.map(i => i.tipo_sangre).filter(Boolean))].forEach(v => {
        const o = document.createElement("option"); o.value = v; o.textContent = v; tipoSangreI.appendChild(o);
    });
}
...
function poblarSelectsInv(data) {
    ...
    [...new Set(data.map(i => i.estatus).filter(Boolean))].forEach(v => {
        const o = document.createElement("option"); o.value = v; o.textContent = v; estadoI.appendChild(o);
    });
    [...new Set(data.map(i => i.tipo_insumo).filter(Boolean))].forEach(v => {
        const o = document.createElement("option"); o.value = v; o.textContent = v; tipoI.appendChild(o);
    });
    [...new Set(data.map(i => i.nombre_presentacion).filter(Boolean))].forEach(v => {
        const o = document.createElement("option"); o.value = v; o.textContent = v; preI.appendChild(o);
    });
}
...
function filtrarMorbilidad() {
    ...
    if (filtered.length === 0) {
        Swal.fire({ icon: "warning", title: "Sin resultados", text: "No se encontraron registros.", confirmButtonText: "Entendido" });
        document.getElementById("contenedor_medicina").style.display = "none";
        tableMed.clear().draw(); lastFilteredMed = [];
        return;
    }
    ...
    Swal.fire({ icon: "success", title: "Reporte generado", text: `Se encontraron ${filtered.length} registro(s) de morbilidad.`, confirmButtonText: "Entendido", timer: 3000, timerProgressBar: true });
}
...
function filtrarInventario() {
    ...
    const filtered = dataInventario.filter(item => {
        ...
        if (presentacion && item.nombre_presentacion !== presentacion) match = false;
        return match;
    });
    ...
    if (filtered.length === 0) {
        Swal.fire({ icon: "warning", title: "Sin resultados", text: "No se encontraron registros.", confirmButtonText: "Entendido" });
        document.getElementById("contenedor_inventario").style.display = "none";
        tableInv.clear().draw(); lastFilteredInv = [];
        return;
    }
    ...
    filtered.forEach(item => {
        ...
        const fechaVenc = dpV.length === 3 ? `${dpV[2]}-${dpV[1]}-${dpV[0]}` : (item.fecha_vencimiento || "N/A");

        const stock = item.cantidad == 0 ? "Agotado" : item.cantidad <= 10 ? "Bajo" : item.cantidad >= 500 ? "Máximo" : "Disponible";
    ...
    Swal.fire({ icon: "success", title: "Reporte generado", text: `Se encontraron ${filtered.length} registro(s) de inventario.`, confirmButtonText: "Entendido", timer: 3000, timerProgressBar: true });
}
...
function actualizarVisibilidadMed() {
    ...
    document.getElementById("wrapper-chartTS").style.display = (sel === 'todos' || sel === 'sangre') ? '' : 'none';
}

function actualizarVisibilidadInv() {
    const sel = document.getElementById("select-grafica-inv").value;
    document.getElementById("wrapper-chartE").style.display = (sel === 'todos' || sel === 'estado') ? '' : 'none';
    document.getElementById("wrapper-chartTI").style.display = (sel === 'todos' || sel === 'tipo_insumo') ? '' : 'none';
    document.getElementById("wrapper-chartPRE").style.display = (sel === 'todos' || sel === 'presentacion') ? '' : 'none';
}
...
function inicializarDTMedicina() {
    const chartMap = [
        ...
        { id: "chartTS", wrapperId: "wrapper-chartTS", title: "Gráfico por Tipo de Sangre" }
    ];
    tableMed = $("#tabla_medicina").DataTable({
        ...
        buttons: [
            { extend: "excelHtml5", text: '<i class="far fa-file-excel"></i> Excel', title: "Reporte Medicina - Morbilidad", className: "btn btn-success btn-sm m-1" },
            { extend: "pdfHtml5", text: '<i class="fas fa-file-pdf"></i> PDF', title: "Reporte Medicina - Morbilidad", className: "btn btn-danger btn-sm m-1", orientation: "landscape", pageSize: "A4", exportOptions: { columns: ":visible" }, customize: crearPdfCustomize(chartMap) }
        ],
        language: langES
    });
...
function inicializarDTInventario() {
    const chartMap = [
        ...
        { id: "chartTI", wrapperId: "wrapper-chartTI", title: "Gráfico por Tipo de Insumo" },
        { id: "chartPRE", wrapperId: "wrapper-chartPRE", title: "Gráfico por Presentación" }
    ];
    tableInv = $("#tabla_inventario").DataTable({
        responsive: true, autoWidth: false, paging: true, lengthChange: true, searching: true, ordering: true, info: true, dom: "Bfrtip",
        buttons: [
            { extend: "excelHtml5", text: '<i class="far fa-file-excel"></i> Excel', title: "Reporte Medicina - Inventario", className: "btn btn-success btn-sm m-1" },
            { extend: "pdfHtml5", text: '<i class="fas fa-file-pdf"></i> PDF', title: "Reporte Medicina - Inventario", className: "btn btn-danger btn-sm m-1", orientation: "landscape", pageSize: "A4", exportOptions: { columns: ":visible" }, customize: crearPdfCustomize(chartMap) }
        ],
        language: langES
    });
...
function renderChartsMed(data) {
    ...
    chartG = new Chart(document.getElementById("chartG").getContext("2d"), buildChartConfig(chartTypeMed, gL, gV, "Género"));
    ...
    chartP = new Chart(document.getElementById("chartP").getContext("2d"), buildChartConfig(chartTypeMed, pL, pV, "PNF"));
    ...
    chartTS = new Chart(document.getElementById("chartTS").getContext("2d"), buildChartConfig(chartTypeMed, tL, tV, "Tipo de Sangre"));
}
...
function renderChartsInv(data) {
    ...
    chartE = new Chart(document.getElementById("chartE").getContext("2d"), buildChartConfig(chartTypeInv, eL, eV, "Estado"));
    ...
    chartTI = new Chart(document.getElementById("chartTI").getContext("2d"), buildChartConfig(chartTypeInv, tiL, tiV, "Tipo de Insumo"));

    // Presentación
    const prU = [...new Set(data.map(i => i.nombre_presentacion).filter(Boolean))];
    const prL = prU.length ? prU : ["N/E"];
    const prV = prU.map(p => data.filter(i => i.nombre_presentacion === p).length);
    if (chartPRE) chartPRE.destroy();
    chartPRE = new Chart(document.getElementById("chartPRE").getContext("2d"), buildChartConfig(chartTypeInv, prL, prV, "Presentación"));
}
