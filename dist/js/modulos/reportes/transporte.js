...
function actualizarInterfaz(cat) {
    document.querySelectorAll(".f-vehiculo, .f-proveedor, .f-ruta, .f-repuesto").forEach(el => el.style.display = "none");
    limpiarResultados();
...
function limpiarResultados() {
    document.querySelectorAll(".cont-rep").forEach(el => el.style.display = "none");
}
...
function cargarData() {
    fetch(BASE_URL + "reportes_transporte_data")
        ...
        .catch(err => {
            console.error(err);
            Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo cargar la data de transporte.' });
        });
}

function extraerFiltros() {
    rawFiltros.v.modelos = [...new Set(rawData.vehiculos.map(i => i.modelo))].filter(Boolean);
    rawFiltros.ru.partidas = [...new Set(rawData.rutas.map(i => i.punto_partida))].filter(Boolean);
    rawFiltros.ru.destinos = [...new Set(rawData.rutas.map(i => i.punto_destino))].filter(Boolean);
    rawFiltros.re.proveedores = [...new Set(rawData.repuestos.map(i => i.nombre_prov))].filter(Boolean);
}
...
function filtrarYRenderizar() {
    ...
    if (!cat) {
        Swal.fire({ icon: 'info', title: 'Atención', text: 'Por favor seleccione un tipo de reporte.' });
        return;
    }
    ...
    limpiarResultados();
    const contMap = { 'vehiculos': 'v', 'proveedores': 'p', 'rutas': 'ru', 'repuestos': 're' };
...
function renderData(key, data) {
    ...
    if (key === 'v') {
        data.forEach(i => table.row.add([formatFecha(i.fecha_adquisicion), i.placa, i.tipo, i.estado]));
    } else if (key === 'p') {
        data.forEach(i => table.row.add([formatFecha(i.fecha_creacion), `${i.tipo_documento}-${i.num_documento}`, i.nombre, i.telefono, i.correo, i.direccion]));
    } else if (key === 'ru') {
        data.forEach(i => table.row.add([formatFecha(i.fecha_creacion), i.nombre_ruta, i.tipo_ruta, i.punto_partida, i.punto_destino, i.horario_salida, i.horario_llegada]));
    } else if (key === 're') {
        data.forEach(i => table.row.add([formatFecha(i.fecha_creacion), i.nombre, i.cantidad, i.nombre_prov, i.estatus]));
    }
    table.draw();
...
function renderCharts(key, data) {
    ...
    if (key === 'v') {
        ...
        c.M = new Chart(document.getElementById("chartVM").getContext("2d"), config("Por Modelo", dM.labels, dM.values));
        c.T = new Chart(document.getElementById("chartVT").getContext("2d"), config("Por Tipo", dT.labels, dT.values));
        c.E = new Chart(document.getElementById("chartVE").getContext("2d"), config("Por Estado", dE.labels, dE.values));
    } else if (key === 'p') {
        const dE = countBy(data, 'estatus');
        c.E = new Chart(document.getElementById("chartPE").getContext("2d"), config("Por Estatus", dE.labels, dE.values));
    } else if (key === 'ru') {
        const dT = countBy(data, 'tipo_ruta');
        const dE = countBy(data, 'estatus');
        const dP = countBy(data, 'punto_partida');
        const dD = countBy(data, 'punto_destino');
        c.T = new Chart(document.getElementById("chartRUT").getContext("2d"), config("Por Tipo", dT.labels, dT.values));
        c.E = new Chart(document.getElementById("chartRUE").getContext("2d"), config("Por Estatus", dE.labels, dE.values));
        c.P = new Chart(document.getElementById("chartRUP").getContext("2d"), config("Puntos Partida", dP.labels, dP.values));
        c.D = new Chart(document.getElementById("chartRUD").getContext("2d"), config("Puntos Destino", dD.labels, dD.values));
    } else if (key === 're') {
        const dP = countBy(data, 'nombre_prov');
        const dE = countBy(data, 'estatus');
        c.P = new Chart(document.getElementById("chartREP").getContext("2d"), config("Por Proveedor", dP.labels, dP.values));
        c.E = new Chart(document.getElementById("chartREE").getContext("2d"), config("Por Estado", dE.labels, dE.values));
    }
}
...
function inicializarDataTables() {
                ...
                { extend: 'excelHtml5', text: '<i class="fas fa-file-excel"></i> Excel', title: title, className: 'btn btn-success btn-sm' },
                {
                    ...
                    customize: function (doc) {
                        ...
                        chartsList.forEach(c => {
                            ...
                            if (canvas) {
                                doc.content.push({ pageBreak: 'before', text: 'Visualización Estadística', alignment: 'center', margin: [0, 20, 0, 10], fontSize: 16, bold: true });
    ...
    tables.v = buildDT('#tabla_v', 'Reporte Vehículos', ['chartVM', 'chartVT', 'chartVE'], '#btn-exp-v');
    tables.p = buildDT('#tabla_p', 'Reporte Proveedores', ['chartPE'], '#btn-exp-p');
    tables.ru = buildDT('#tabla_ru', 'Reporte Rutas', ['chartRUT', 'chartRUE', 'chartRUP', 'chartRUD'], '#btn-exp-ru');
    tables.re = buildDT('#tabla_re', 'Reporte Repuestos', ['chartREP', 'chartREE'], '#btn-exp-re');
}
