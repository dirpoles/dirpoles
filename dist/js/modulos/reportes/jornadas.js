...
function cargarData() {
    fetch(BASE_URL + "reportes_jornadas_data")
        ...
        .catch(err => {
            console.error(err);
            Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo cargar la data de jornadas.' });
        });
}
...
function filtrarYRenderizar() {
    ...
    if (filtered.length === 0) {
        Swal.fire({ icon: 'warning', title: 'Sin resultados', text: 'No se encontraron jornadas con esos filtros.' });
        document.getElementById("contenedor_general").style.display = "none";
        return;
    }
...
function renderCharts(data) {
    ...
    charts.T = new Chart(document.getElementById("chartT").getContext("2d"), config("Tipo", dT.labels, dT.values));
    charts.U = new Chart(document.getElementById("chartU").getContext("2d"), config("Ubicación", dU.labels, dU.values));
...
function actualizarVisibilidadCharts() {
    ...
    else if (v === "tipo") document.getElementById("wrapper-chartT").style.display = "";
    else if (v === "ubicacion") document.getElementById("wrapper-chartU").style.display = "";
    else if (v === "estado") document.getElementById("wrapper-chartE").style.display = "";
}
...
function inicializarDataTable() {
    tableGeneral = $("#tabla_general").DataTable({
        ...
        dom: 'Bfrtip',
        buttons: [
            { extend: 'excelHtml5', text: '<i class="fas fa-file-excel"></i> Excel', title: 'Reporte Jornadas', className: 'btn btn-success btn-sm' },
            {
                extend: 'pdfHtml5', text: '<i class="fas fa-file-pdf"></i> PDF', title: 'Reporte Jornadas', className: 'btn btn-danger btn-sm',
                ...
                customize: function (doc) {
                    ...
                    const visibleCharts = [
                        { id: "chartT", wrap: "wrapper-chartT", t: "Distribución por Tipo" },
                        { id: "chartU", wrap: "wrapper-chartU", t: "Distribución por Ubicación" },
                        { id: "chartE", wrap: "wrapper-chartE", t: "Distribución por Estado" }
                    ];
