...
function editarEquipo(id) {
    ...
    $('#modalGenericoTitle').text('Editar Equipo');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const e = data.equipo;
            const tipos = data.tipos_equipo || [];
            const servicios = data.servicios || [];

            let optionsTipos = '<option value="" disabled>Seleccione un tipo</option>';
            tipos.forEach(function(t) {
                const selected = t.id_tipo_equipo == e.id_tipo_equipo ? 'selected' : '';
                optionsTipos += `<option value="${t.id_tipo_equipo}" ${selected}>${t.nombre}</option>`;
            });

            let optionsServicios = '<option value="" disabled>Seleccione un servicio</option>';
            servicios.forEach(function(s) {
                const selected = s.id_servicios == e.id_servicios ? 'selected' : '';
            ...
            const modalContent = `
                <form id="formEditarEquipo">
                        ...
                        <div class="card-body p-4">
                            <div class="row">
                                ...
                                <div class="col-md-6 border-end">
                                    ...
                                    <div class="mb-3">
                                        <label for="editar_tipo_equipo" class="form-label text-muted small mb-1">
                                            <i class="fas fa-tag text-primary me-1"></i> Tipo de Equipo <span class="text-danger">*</span>
                                        </label>
                                        <select class="form-select form-select-sm" id="editar_tipo_equipo" name="id_tipo_equipo" required>
                                            ${optionsTipos}
                                        </select>
                                        <div class="text-danger form-text" id="editar_tipo_equipoError"></div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control form-control-sm" id="editar_marca" name="marca" value="${e.marca || ''}" required>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control form-control-sm" id="editar_modelo" name="modelo" value="${e.modelo || ''}" required>
                                        <div class="text-danger form-text" id="editar_modeloError"></div>
                                    </div>

                                    <div class="mb-3">
                                        <label for="editar_serial" class="form-label text-muted small mb-1">
                                            <i class="fas fa-barcode text-primary me-1"></i> Serial <span class="text-danger">*</span>
                                        </label>
                                        <input type="text" class="form-control form-control-sm" id="editar_serial" name="serial" value="${e.serial || ''}" required>
                                        <div class="text-danger form-text" id="editar_serialError"></div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control form-control-sm" id="editar_color" name="color" value="${e.color || ''}">
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control form-control-sm" id="editar_fecha_adquisicion" name="fecha_adquisicion" value="${e.fecha_adquisicion || ''}" required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select form-select-sm" id="editar_estado" name="estado" required>
                                            ...
                                            <option value="Nuevo" ${e.estado === 'Nuevo' ? 'selected' : ''}>Nuevo</option>
                                            ...
                                            <option value="En reparación" ${e.estado === 'En reparación' ? 'selected' : ''}>En reparación</option>
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control form-control-sm" id="editar_descripcion" name="descripcion" rows="3">${e.descripcion || ''}</textarea>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control form-control-sm" id="editar_observaciones" name="observaciones" rows="2">${e.observaciones || ''}</textarea>
                                    </div>
                                </div>
                            </div>

                            <input type="hidden" name="id_equipo" id="id_equipo" value="${e.id_equipo || id}">
                        </div>
                    </div>
                    ...
                    <div class="modal-footer border-top-0 bg-light py-3">
                        ...
                        <button type="submit" class="btn btn-primary" id="btnGuardarCambiosEquipo">
            ...
            inicializarValidarEditarEquipo(id);
        },
            ...
            $('#modalGenerico .modal-body').html(`
                <div class="alert alert-danger m-4">
                    ...
                    <div class="mt-3 text-center">
                        <button class="btn btn-outline-danger" onclick="editarEquipo(${id})">
...
/**
 * Inicializa la validación y envío del formulario de edición de equipo
 */
function inicializarValidarEditarEquipo(id) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!formData.get('id_tipo_equipo')) {
            $('#editar_tipo_equipoError').text('Debe seleccionar un tipo de equipo');
            errores.push('tipo');
        }
        ...
        if (!formData.get('serial') || formData.get('serial').trim() === '') {
            $('#editar_serialError').text('El serial es obligatorio');
            errores.push('serial');
        }
        ...
        const btnGuardar = $('#btnGuardarCambiosEquipo');
        btnGuardar.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Guardando...');

        $.ajax({
            url: 'equipo_actualizar',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function(response) {
                if (response.exito) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Actualizado',
                        text: response.mensaje || 'Equipo actualizado exitosamente',
                        timer: 1500,
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: response.mensaje || 'Error al actualizar el equipo',
                        confirmButtonText: 'Entendido'
                    });
