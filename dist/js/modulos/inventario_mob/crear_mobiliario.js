window.onload = function () {
  ...
  const firstRowRemoveBtn = container.querySelector(
    ".item-row-mobiliario .btn-remove-row",
  );
  ...
  function attachEvents(row) {
    ...
    $(inputs.id_tipo_mobiliario).on(
      "change.select2 select2:select",
      function () {
    ...
    inputs.fecha_adquisicion.addEventListener("change", () =>
    ...
    inputs.observaciones.addEventListener("input", () =>
    ...
    if (btnAdd) {
      ...
      if (currentRows >= 10) {
        AlertManager.warning(
          "Límite alcanzado",
          "No puedes registrar más de 10 mobiliarios al mismo tiempo.",
        );
        return;
      }
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (validarTodo()) {
      try {
        ...
        if (data.exito) {
          AlertManager.success(
            "Registro exitoso",
            data.mensaje || "Mobiliario registrado correctamente",
          ).then(() => {
          ...
          AlertManager.error(
            "Error",
            data.mensaje || "No se pudo registrar el mobiliario",
          );
        }
      } catch (error) {
      ...
      AlertManager.warning(
        ...
        "Por favor, rellene todos los campos obligatorios correctamente",
      );
    }
  });
};
