function inicializarValidacionesEditarRepuesto() {
...
async function enviarFormularioRepuesto(form) {
    try {
        ...
        const response = await fetch('repuesto_actualizar', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (data.exito || data.success) {
            AlertManager.success('Exito', data.mensaje || 'Repuesto actualizado correctamente.');
            ...
            AlertManager.error('Error', data.mensaje || 'No se pudo actualizar el repuesto.');
        }
